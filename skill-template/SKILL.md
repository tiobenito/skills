---
name: skill-template
description: Package a Claude Code skill into a marketplace-ready template and stage it to a target repo for publishing. Use when the user says "publish this skill", "templatize this skill", "ship this skill to the marketplace", "turn this skill into a template", "make this skill installable", or wants to take an existing skill (or a workflow idea) and prepare it as an installable plugin. Orchestrates skill-creator and visualization; does not replace them.
---

# Skill Template

Turn a skill (or a workflow idea) into a clean, **marketplace-ready template** and stage it
to a target repo. The deliverable is always a template — a generic, packaged skill.
Publishing is the last step, and this skill stops before the push.
...
Keep this lean. Detailed formats and per-target rules live in `references/` — read each one
when its step calls for it.

## Step 0 — Pick the target

Ask which target this skill is for. Fix the answer before anything else — it drives the
strip policy, the deck branding, and the publish destination.

| Target | For | Reference file |
|---|---|---|
| **Public** | A public, generic skill for anyone to install | `references/targets/public.md` |
| **Personal** | A skill just for you, packaged + versioned | `references/targets/personal.md` |

Read the chosen target file now. It pins the repo, the GitHub account, the strip policy,
and the deck design system. **Never guess the GitHub account** — it is set in the target
file. If you keep separate work and personal GitHub identities, follow your own routing
rules so you never cross-push.
...
directly into the target repo. Step 4 branches the target repo and moves the finished
skill in. This keeps "no writes to the target repo before the branch" literally true, and
an abandoned run leaves only a throwaway temp dir.

### Mode A — Templatize an existing skill

1. Read the whole source skill — `SKILL.md`, every file in `references/`, `examples/`,
   and `learnings.md`.
2. Run the **detect-personal pass**: read `references/detect-personal.md` and follow it.
   It is target-aware — what gets stripped depends on Step 0.
...
that just works on install can be covered by a README section instead — but the user
decides.
...
2. Hand off to the **`visualization`** skill — do not build HTML directly. Pass it the
   outline and the design system named in the target file (a neutral template theme for
   public skills; your own brand palette for personal ones if you have one).
3. Output: `<name>-guide.html`, built into the staging directory. Step 4 places it at the
   target repo root.

## Step 4 — Stage to the target

Read `references/plugin-manifest-formats.md` and the chosen target file, then:

1. **Branch first.** Create a feature branch in the target repo (`add-skill-<name>`)
...
2. Move the staged skill into `plugins/<name>/skills/<name>/`, create the plugin manifest,
   ...
   and the target file.
3. **Validate** with `--strict`, on **both** the marketplace root (checks
   `marketplace.json`) and the new plugin dir `plugins/<name>` (checks `plugin.json`).
...
- **Never cross-push.** The GitHub account is pinned per target file. If you keep separate
  work and personal GitHub identities, never use one account's tooling on the other's repo.
- **Never ship secrets.** Tokens, env var names, and account slugs are stripped for every
  target, regardless of strip policy.
- **Orchestrate, don't reimplement.** `skill-creator` builds skills; `visualization`
  builds decks. This skill sequences them and handles the packaging layer.
